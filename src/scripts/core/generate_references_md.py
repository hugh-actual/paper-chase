...
def extract_surname(author_str):
    ...
    if not author_str or author_str.startswith("---") or author_str == "Unknown":
...
def generate_markdown():
    ...
    if not config.REFERENCES_JSON.exists():
        print(f"❌ {config.REFERENCES_JSON} not found")
        return False

    with open(config.REFERENCES_JSON, "r", encoding="utf-8") as f:
        entries = json.load(f)

    print(f"Loaded {len(entries)} entries from {config.REFERENCES_JSON}")

    # Sort by first author's surname, then title (conventional bibliography order)
    sorted_entries = sorted(
        entries,
        key=lambda e: (
            extract_surname(e["author"]).lower(),
            e["title"].lower() if e["title"] else "",
        ),
    )
    ...
    # Write to file
    with open(config.REFERENCES_FILE, "w", encoding="utf-8") as f:
        f.writelines(lines)
